...
С приходом ES6, впервые за долгое время появился новый примитивный тип в JavaScript: `символ`. Однако, в отличие от других примитивов, у него нет своей литеральной формы.

Вот пример создания символа:
...
Некоторый вещи нужно запомнить:

- Вы не должны использовать `new` перед `Symbol(..)`. Это не конструктор и в результате получится не объект.
- Параметр внутри `Symbol(..)` опционален и служит для описания смысловой нагрузки символа.
- `typeof` выводит `"symbol"` и это главный способ определения, что значение перед вами имеет такой тип.

Описание служит единственной цели -- представлении символа в виде строке:
...
Главный смысл символа -- создать строко-подобное значение, которое не будет совпадать ни с каким иным значением. Положим, мы используем символ как константу для названия некоторого события:
...
Представим себе такой модуль, реализующий шаблон _синглтон_:
...
`Symbol.for(..)` ищет в глобальном реестре символов тот, что содержит строку из описания и возвращает его. Если такой не найден -- он создается. Другими словами, каждый символ в глобальном реестре символов является синглтоном, то есть может быть один и только один символ с определенным описанием.
