...
Как уже упоминалось в главе 1, дискуссии о том, чем является приведение -- полезной функциоанльностью или изъяном языка, не потеряли своей ожесточенности по сей день. Если вы читали другей популярные книги по JS, вы могли заметить, что подавляещее большинство мнений сходится в том, что приведение это нечто таинственное, магическое, злое, смущающее или нечто, использование чего -- априори плохая идея.
...
Для `b` приведение было неявным, поскольку оператор`+` при наличии строкового операнда (`""`) трактуется как соединение сток, _такой вот сайд-эффект_ приводит к насильственному приведению числа `42` к его строковому эквиваленту: `"42"`.

Контрастирующей является функция `String(..)` которая самоочевидным образом принимая на вход числовое значение на выходе отдаст строковое представление.

Оба подхода дают один результат: `42`становится `"42"`. But it's the _how_ that is at the heart of the heated debates over JavaScript coercion.

**Замечание:** Technically, there's some nuanced behavioral difference here beyond the stylistic difference. We cover that in more detail later in the chapter, in the "Implicitly: Strings <--> Numbers" section.
...
For regular objects, unless you specify your own, the default `toString()` (located in `Object.prototype.toString()`) will return the _internal `[[Class]]`_ (see Chapter 3), like for instance `"[object Object]"`.
...
**Замечание:** The way an object is coerced to a `string` technically goes through the `ToPrimitive` abstract operation (ES5 spec, section 9.1), but those nuanced details are covered in more detail in the `ToNumber` section later in this chapter, so we will skip over them here.

У массивов есть свой переопределенный метод `toString()` который по умолчанию соединяет все свои элементы(также приведенные к строке) в единую строку, с разделителем `","`:
...
Повторюсь, метод `toString()` может быть вызван как явно, так инеявно, если операция совершается в контексте, предполагающем строки.
...
For most simple values, JSON stringification behaves basically the same as `toString()` conversions, except that the serialization result is _always a `string`_:
...
Например, `true` становится `1` а `false` -- `0`. `undefined` становится `NaN`, однако (внезапно) `null` -- `0`.
...
**Замечание:** The differences between `number` literal grammar and `ToNumber` on a `string` value are subtle and highly nuanced, and thus will not be covered further here. Consult section 9.3.1 of the ES5 spec for more information.
...
**Замечание:** We cover how to coerce to `number`s later in this chapter in detail, but for this next code snippet, just assume the `Number(..)` function does so.
...
- `null`
- `false`
- `+0`, `-0`, and `NaN` ( также простой `0`, прим. переводчика)
- `""`
...
**Замечание:** That's a subtle specification joke some of you may get.
...
There are certain cases where browsers have created their own sort of _exotic_ values behavior, namely this idea of "falsy objects," on top of regular JS semantics.
...
**Замечание:** The generally accepted perspective in the open-source JS community is that unary `+` is an accepted form of _explicit_ coercion.
...
**Замечание:** Another extremely confusing place for unary `+` to be used adjacent to another operator would be the `++` increment operator and `--` decrement operator. For example: `a +++b`, `a + ++b`, and `a + + +b`. See "Expression Side-Effects" in Chapter 5 for more about `++`.
...
**Замечание:** This is very similar to how `!` not only coerces its value to `boolean` but also flips its parity (see discussion of the "unary `!`" later).
...
**Замечание:** The `-(x+1)` pseudo-algorithm for `~` would imply that `~-1` is `-0`, but actually it produces `0` because the underlying operation is actually bitwise, not mathematic.
...
**Замечание:** The bitwise double-flip of `~~` is very similar to the parity double-negate `!!` behavior, explained in the "Explicitly: \* --> Boolean" section later.
...
**Замечание:** A commonly cited coercion gotcha is `[] + {}` vs. `{} + []`, as those two expressions result, respectively, in `"[object Object]"` and `0`. There's more to it, though, and we cover those details in "Blocks" in Chapter 5.
...
**Замечание:** Of course, instead of the `for` loop in `onlyOne(..)`, you could more tersely use the ES5 `reduce(..)` utility, but I didn't want to obscure the concepts.
...
**Замечание:** Regardless of _implicit_ or _explicit_ approaches, you could easily make `onlyTwo(..)` or `onlyFive(..)` variations by simply changing the final comparison from `1`, to `2` or `5`, respectively. That's drastically easier than adding a bunch of `&&` and `||` expressions. So, generally, coercion is very helpful in this case.
...
Any value used in these contexts that is not already a `boolean` will be _implicitly_ coerced to a `boolean` using the rules of the `ToBoolean` abstract operation covered earlier in this chapter.
...
In fact, I would argue these operators shouldn't even be called "logical \_\_\_ operators", as that name is incomplete in describing what they do. If I were to give them a more accurate (if more clumsy) name, I'd call them "selector operators," or more completely, "operand selector operators."
...
**Замечание:** Both the `a = b || "something"` and `a && b()` idioms rely on short circuiting behavior, which we cover in more detail in Chapter 5.
...
**Замечание:** The implication here then is that both `==` and `===` check the types of their operands. The difference is in how they respond if the types don't match.
...
**Замечание:** The `===` strict equality comparison is defined identically to 11.9.3.1, including the provision about two `object` values. It's a very little known fact that **`==` and `===` behave identically** in the case where two `object`s are being compared!
...
**Замечание:** The `!=` loose not-equality operation is defined exactly as you'd expect, in that it's literally the `==` operation comparison performed in its entirety, then the negation of the result. The same goes for the `!==` strict not-equality operation.
...
In my opinion, the form `a == null` is yet another example where _implicit_ coercion improves code readability, but does so in a reliably safe way.
...
**Замечание:** You may notice that these clauses only mention `String` and `Number`, but not `Boolean`. That's because, as quoted earlier, clauses 11.9.3.6-7 take care of coercing any `Boolean` operand presented to a `Number` first.
...
Oooo, that seems at a higher level of crazy, right!? Your brain may likely trick you that you're comparing a truthy to a falsy value, so the `true` result is surprising, as we _know_ a value can never be truthy and falsy at the same time!
...
In the overall scheme of things, there's relatively few cases where _implicit_ coercion is truly dangerous. But in those places, for safety sake, definitely use `===`.
...
**Замечание:** The algorithm is only defined for `a < b`. So, `a > b` is handled as `b < a`.
...
**Замечание:** Similar caveats for `-0` and `NaN` apply here as they did in the `==` algorithm discussed earlier.
